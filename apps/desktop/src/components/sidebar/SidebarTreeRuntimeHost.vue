...
async function toggle(requestId = beginNavigationRequest()) {
  ...
  if ((node.type === "group-extensions" || node.type === "group-foreign-servers" || node.type === "group-event-triggers") && connectionStore.canUseLoadedTreeNodeToggle(node)) {
    node.isExpanded = !node.isExpanded;
    if (wasExpanded && shouldReleaseCollapsedTreeNodeChildren()) connectionStore.releaseCollapsedTreeNodeChildren(node.id);
    emitNodeToggled(node, wasExpanded);
    return;
  ...
  try {
    ...
    } else if ((node.type === "group-foreign-servers" || node.type === "group-event-triggers") && node.connectionId && hasTreeNodeDatabaseContext(node)) {
      await connectionStore.refreshTreeNode(node);
    }
    emitNodeToggled(node, wasExpanded);
  } catch (e: any) {
