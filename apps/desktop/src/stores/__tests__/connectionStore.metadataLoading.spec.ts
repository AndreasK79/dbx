...
describe("connectionStore metadata loading", () => {
  ...
  it("clears a stale connection error after a schema metadata retry succeeds", async () => {
    ...
    expect(store.treeNodes[0]?.children?.[0]?.children?.map((node) => node.label)).toEqual(["public", "tree.extensions", "tree.foreignServers", "tree.queries"]);
  });
  ...
  it("does not let an older refresh resume after a newer refresh succeeds", async () => {
    ...
    expect(databaseNode.children?.map((node) => node.label)).toEqual(["latest", "tree.extensions", "tree.foreignServers", "tree.queries"]);
  });
