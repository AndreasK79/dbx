...
export function treeNodeLoadedChildrenContentPresent(node: TreeNode, sidebarObjectDisplay: SidebarObjectDisplayMode): boolean {
  ...
  if (objectTypesForGroupNode(node.type) || TABLE_STRUCTURE_GROUP_TYPES.has(node.type) || node.type === "group-extensions" || node.type === "group-foreign-servers" || node.type === "group-event-triggers" || node.type === "group-tablespaces" || node.type === "group-datafiles") {
    return true;
  }
  return childCount > 0;
}
